...
const HEROES = [
  ...
  { id:'K',   tiles:['風刃','風壓','治癒'], color:'#a06a3c', dark:'#5e3f23' }, // 1,4,3（暖棕木色＝廚子；不走 Nord 橘那種偏紅）
...
function applyHeroColor(){ const h = HEROES[0]; h.color = settings.heroColor; h.dark = darken(settings.heroColor); document.documentElement.style.setProperty('--hero-color', settings.heroColor); }
...
// ---------- 遊戲狀態 ----------
const VERSION = 'v0.2.56'; // 語意化版本 主.次.修：次號留給大里程碑、日常小改用修號；粗胚維持 0.x（規則見 CLAUDE.md）
...
const BATTLES = {
  ...
  boss:  { tier:'👑 Boss 戰', intro:'淵蟲王從深處升起。', make:()=>[
    makeWorm('淵蟲王', ['normal','normal','heavy','normal','resonance','normal','heavy'],
...
// ---------- 出擊 ----------
let animating = false;
// 出擊按鈕的亮粉火花特效：一圈粉光暈＋幾點噴出的亮粉小點（transform/opacity＝不吃效能）
function attackSparkle(){
  ...
  for(let i=0;i<9;i++){
    ...
    const ang = (Math.PI*2/9)*i + Math.random()*0.5, dist = 24 + Math.random()*18;
...
function attack(){
  ...
  setTimeout(()=>{
   try {
    ...
    if(isFull){ goldBurst(); floatNum('✨ 滿順 ✨', 'full'); setTimeout(()=> floatNum('體力全退 +15', 'stam', 'team'), 360); log('✨ <b style="color:#ffe082">滿順</b>！體力全額退還（+15）'); }
...
function tileEl(t, onClick){
  ...
  d.innerHTML = `<span class="num">${t.skill.num}</span><span class="nm">${t.skill.name}</span><span class="who">${displayName(t.who)}</span><span class="eff">${descHtml}</span>`
...
function render(){
  ...
  allInBtn.textContent = allIn ? 'ALL IN！' : '點擊以 ALL IN'; // 沒按＝提示可點；按了＝ALL IN！
